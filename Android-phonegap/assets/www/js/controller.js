 ...
 */
$('#home-nav-menu-wrapper').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend',   
    function(e) {
    	$('#home-nav-menu-wrapper').addClass('home')
    }
);
$(document).bind('pagebeforeshow', function(event) {
	...
	$('#home-nav-menu-wrapper').removeClass('home');
...
/*		if (auxPageId == 'calendar') {
			...
			var frame = '<iframe src="' + 'https://www.google.com/calendar/embed?' + 'showTitle=0&amp;showNav=0&amp;showDate=0&amp;showPrint=0&amp;showCalendars=0&amp;mode=AGENDA&amp;height=600&amp;wkst=1&amp;bgcolor=%23FFFFFF' + '&amp;src=' + window.settings.calendar + '&amp;color=%23A32929&amp;ctz=America%2FBogota' + '&dates=' + window.settings.beginDate + '%2F' + window.settings.endDate +'" style="border-width:0;" width="100%" height="100%" frameborder="0" scrolling="no"></iframe>';
...
function setMenuRotation(rotation, top) {
	...
	$('#home-nav-menu-wrapper').css({
		WebkitTransform : ' rotateZ(' + (-rotation) + 'deg) translate3d(0,' + top + ',0)',
		MsTransform : ' rotateZ(' + (-rotation) + 'deg) translate3d(0,' + top + ',0)',
		transform : ' rotateZ(' + (-rotation) + 'deg) translate3d(0,' + top + ',0)',
		zoom : zoom
	});
